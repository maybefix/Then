...
- Canvas テキストカードをすべて並べ替え画面へ出し、不要なカードは中間稿の生成対象から除外できる。
- 除外したカードは一覧に薄く残し、その場で生成対象へ戻せる。Canvas本体のカードは削除・変更しない。
...
- 接続方向から作った初期順序を提示する。
- ドラッグ操作で人が最終順序を決める。
- 各行の除外操作で、そのカードを生成対象から外したり戻したりできる。
...
- 空のテキストカードも並べ替え一覧・スナップショットへ含める。空白のみのカードと明示的に除外したカードは生成本文から除く。生成対象に本文がなければ生成を無効にする。
- グループ名と資料カードは生成対象に含めない。カード本文の1行目も通常の本文として扱う。
- `arrow`（旧データの種別省略も含む）の方向でトポロジカル順序を作る。`line`、`bidirectional`、`dashed` は順序を指定しない。
- 候補が複数ある場合は上から下、左から右、ID順。循環で候補がなくなった場合もこの順で一つ選び、全カードを一度ずつ並べる。最終順序はドラッグまたは上下ボタンで人が確定する。
- 空でないカード本文を変更せず、間に改行2文字を挿入して連結する。
- 編集後の並べ替えは、Canvasを取得し直して新バージョンを生成する。既存テキストを並べ替えで書き換えない。過去版は参照専用、最新版は編集可能。
- 作品ボードは `.then/boards/.intermediate-drafts/<boardId>/`、共通ボードはアプリデータ内の `boards/.intermediate-drafts/<boardId>/` に保存する。
- `history.json` にボードID、スキーマ、保存リビジョン、版ごとの取得・作成・更新日時、カードID・本文スナップショット、順序、除外カード、編集後本文を保持する。各版の本文はUTF-8の `v1.txt`、`v2.txt` …にも保存する。Thenでの再読込は `history.json` を正本とするため、外部エディタによる `.txt` の変更は自動で取り込まない。
- 保存はボード単位で直列化し、リビジョン競合時は上書きしない。保存失敗時は画面内の編集を維持し、再試行・全文コピー・保存版の再読込を用意する。
- ボードのゴミ箱移動・復元には中間稿の保存ディレクトリも追従する。
- 版の削除・複製・名称付けは今回含めない。
- 本文画面でも所属ボード選択に「作品／共通」を表示し、Global Canvasの中間稿を選べる。別ウィンドウCanvasの「稿」ボタンからも同じペインを開ける。
- 本文と中間稿のスクロール・キー操作は独立する。Canvasカードと並べ替え行の選択を連動させ、行からカードを画面中央へ表示できる。

検証: `npm run test:v063-draft`、`npm run build`、Rustの `intermediate_draft` テスト。ブラウザで別ウィンドウCanvasの生成と本文モードからの参照・レイアウトを確認。
