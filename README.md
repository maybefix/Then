...
- 通常の入力操作ではProseMirrorのトランザクションから変更行を直接特定し、長文中の編集で先頭・末尾から全段落を比較する処理を減らしました。
- ファイルやタブを切り替えた直後の最初の編集が破棄される可能性を解消し、Tauriイベントの登録・解除競合によるリスナー残留を防ぎました。
- タイプライタースクロールの固定位置を示す中央線だけを設定から非表示にできるようにしました。
