...
- 全画面を起動時から利用可能な静的importのまま、React、Tiptap / ProseMirror、Tauriを個別のvendor chunkへ分割しました。main chunkは約824.7 KiBから約392.7 KiBへ縮小し、通常JavaScript chunkをすべて500 kB未満にしています。PDF / DOCXの既存オンデマンド読み込みは維持します。
- 起動時JavaScript総量は約845.7 kBでほぼ不変です。画面の初回利用を待たせないことを優先したchunk分割であり、初回転送総量そのものの削減は今後の課題です。
